...
const BlogCard: React.FC<BlogCardProps> = ({ post, index = 0 }) => {
  ...
  const [viewCount, setViewCount] = useState(0); // Initialize with 0
  ...
  // Generate random values only after component mounts on client
  useEffect(() => {
    setIsMounted(true);
        ...
        <CardHeader className="p-0 mb-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-4 text-xs text-gray-500">
              ...
              {/* Only render stats after client-side mount */}
              {isMounted ? (
                <>
                  <div className="flex items-center gap-1">
                    <Eye size={12} />
